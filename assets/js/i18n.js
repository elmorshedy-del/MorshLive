...
(function () {
  "use strict";
  const stamp = "20260904railwaydiag1";
  ...
  function localMatchTimeZones(match) {
    ...
    return [
      { key: "local", label: lang === "en" ? "Your local time" : "توقيتك المحلي", shortLabel: lang === "en" ? "Local" : "محلي", value: format(), timeZone: zone },
      { key: "makkah", label: lang === "en" ? "Makkah time" : "بتوقيت مكة", shortLabel: lang === "en" ? "Makkah" : "مكة", value: format("Asia/Riyadh"), timeZone: "Asia/Riyadh" },
    ];
  }

  try {
    Object.defineProperty(window, "getMatchTimeZones", { configurable: true, get() { return localMatchTimeZones; }, set() {} });
  } catch (_) { window.getMatchTimeZones = localMatchTimeZones; }
  ...
  function writeSharedAssets() {
    ...
    document.write(`<script src="/assets/js/railway-freeze-diagnostics.js?v=${stamp}"><\/script>`);
  ...
  if (document.readyState === "loading") { writeSharedAssets(); return; }

  const addCss = (href) => { const link = document.createElement("link"); link.rel = "stylesheet"; link.href = href; document.head.appendChild(link); };
  const addScript = (src, onload) => { const script = document.createElement("script"); script.src = src; if (onload) script.onload = onload; document.head.appendChild(script); };

  addCss(`/assets/css/dark-refresh.css?v=${stamp}`);
  addCss(`/assets/css/content-architecture.css?v=${stamp}`);
  addScript(`/assets/js/railway-freeze-diagnostics.js?v=${stamp}`);
      ...
      addScript(`/assets/js/content-architecture.js?v=${stamp}`, () => {
        addScript(`/assets/js/arabic-editorial.js?v=${stamp}`, () => {
          addScript(`/assets/js/watch-arabic-editorial.js?v=${stamp}`, () => {
            addScript(`/assets/js/english-editorial.js?v=${stamp}`, () => addScript(`/assets/js/match-stats-editorial.js?v=${stamp}`));
          });
        });
      });
    });
            ...
            addScript(`/assets/js/iptv-legacy-toggle-normalizer.js?v=${stamp}`, () => {
              addScript(`/assets/js/iptv-auto.js?v=${stamp}`, () => {
                addScript(`/assets/js/iptv-stage-copy.js?v=${stamp}`, () => {
                  addScript(`/assets/js/iptv-premium-card-click.js?v=${stamp}`, () => window.__KZ_RELEASE_IPTV_MUTATION_GUARD?.());
                });
              });
            });
          });
