/* /search page — searches current fixtures and routes each result to the correct product surface. */
(function () {
  "use strict";
  ...
  const teamLabel = (name) => (window.TeamNames ? window.TeamNames.localize(name) : name);
  ...
  function currentRecapHref(m) {
    const id = m.id || m.key || (window.TeamNames?.canonicalKey?.(m.home, m.away));
    return id ? `/highlights.html?match=${encodeURIComponent(id)}` : "/highlights.html";
  }

  function watchHref(m) {
    if (m.status === "ended") {
      if (isWorldCupMatch(m)) {
        const href = window.TeamNames?.matchPageHref?.(m);
        if (href) return href;
        return m.key ? `/tournament?match=${encodeURIComponent(m.key)}` : "/tournament";
      }
      return currentRecapHref(m);
    }
    return m.channelId
  ...
  function commentatorText(m) {
    if (m.commentators && m.commentators.length) return m.commentators.map((c) => c.name).join(" ");
  ...
  function timeZoneChips(m) {
    ...
    return `<div class="time-zone-row">${zones.map((z) => `
      <div class="time-chip time-chip-${z.key}"><span>${z.label}</span><b>${z.value}</b></div>`).join("")}</div>`;
  }
  ...
  function matchCard(m) {
    const minute = window.liveMinuteLabel
      ? window.liveMinuteLabel(m)
      : (m.status === "live" && m.minute ? String(m.minute).trim() : "");
    return `
      <article class="match-card" data-status="${m.status}" data-match-id="${m.id || ""}">
        <div class="match-top">
          <span class="league-tag">${leagueLabel(m)}</span>
          <span class="status-pill status-${m.status}">${statusLabel(m.status)}${minute ? ` · ${minute}` : ""}</span>
        </div>
        <div class="teams">
        ...
        ${timeZoneChips(m)}
        <div class="match-foot"><span class="match-meta">${footMeta(m)}</span>${watchAction(m)}</div>
      </article>`;
  }
