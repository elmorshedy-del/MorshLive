...
export function isExplicitContentKey(key) {
  const value = String(key || "");
  return Boolean(value) && !value.startsWith("channel:") && !value.startsWith("match:");
}
...
export function normalizeSource(raw, planContentKey = "", now = Date.now()) {
  ...
  if (kind === "xtream") {
    if (!raw.portalId || !String(raw.streamId || "")) return null;
  } else if (!url && !path) {
    return null;
  }
  const source = {
    ...
    url,
    path,
    fallbackUrl: String(raw.fallbackUrl || "").trim(),
    portalId: raw.portalId ? String(raw.portalId) : "",
    streamId: raw.streamId != null ? String(raw.streamId) : "",
    contentKey,
