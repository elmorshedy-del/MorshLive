...
import { isOperatorAlbaPlayerUrl, operatorEmbedProxyPath } from "./operator-embed.js";
...
export const PROVIDER_PROFILES = Object.freeze({
  ...
  "operator-iframe-v2": {
    id: "operator-iframe-v2",
    provider: "operator",
    kind: "iframe",
    sandbox: "allow-scripts allow-same-origin allow-presentation allow-forms",
    referrerPolicy: "no-referrer",
    notes: "Same-origin /wk/operator proxy: extract inner HLS embed, never the WP popunder chrome",
  },
  "operator-hls-v1": {
    id: "operator-hls-v1",
    provider: "operator",
...
export function playbackUrlForSource(source, context = {}) {
  ...
  if (source.url) {
    const proxied = operatorEmbedProxyPath(source.url, context.origin || "");
...
function resolvedView({ match, plan, selected, status, reason, catalog }) {
  const profile = selected
    ? iframePolicyForProfile(isOperatorAlbaPlayerUrl(selected.url) ? "operator-iframe-v2" : selected.profile)
    : null;
  return {
