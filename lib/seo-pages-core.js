...
export const LEAGUE_META = Object.freeze({
  epl: {
    ...
    descriptionAr:
      "مباريات الدوري الإنجليزي اليوم مباشر مع المواعيد بتوقيت السعودية والنتائج والقنوات والمعلّقين وروابط تفاصيل كل مباراة على كورة زيرو.",
  },
  laliga: {
    slug: "la-liga",
    ...
    descriptionAr:
      "مباريات الدوري الإسباني اليوم مباشر مع المواعيد بتوقيت السعودية والنتائج والقنوات والمعلّقين وروابط تفاصيل كل مباراة على كورة زيرو.",
  },
  ucl: {
    ...
    descriptionAr:
      "مباريات دوري أبطال أوروبا القادمة والجارية والمنتهية مع المواعيد بتوقيت السعودية والنتائج والقنوات والمعلّقين على كورة زيرو.",
  },
});
...
<body>
  <main class="seo-wrap">
    ...
    ${body}
    <footer class="seo-footer">المواعيد معروضة بتوقيت الرياض، وتُحدّث بيانات النتائج والقنوات والمعلّقين من نفس مصدر جدول <a href="/">كورة زيرو</a>.</footer>
  </main>
</body>
</html>`;
}
  ...
  return `<section class="seo-section" aria-labelledby="seo-leagues-title">
    ...
    <div class="seo-leagues">
      ${Object.values(LEAGUE_META)
        .map(
          (meta) => `<a class="seo-league" href="/league/${escapeHtml(meta.slug)}"><strong>${escapeHtml(meta.nameAr)}</strong><span>المواعيد · النتائج · القنوات</span></a>`,
        )
        .join("\n")}
    </div>
  </section>`;
...
export function buildSeoPages(payload, options = {}) {
  ...
  const hubBody = [
    renderLeagueStrip(),
    renderMatchSection("مباريات اليوم", todayMatches, teamNamesAr, `بتوقيت الرياض · ${today}`),
    tomorrowMatches.length
      ? renderMatchSection("مباريات الغد", tomorrowMatches, teamNamesAr, `بتوقيت الرياض · ${tomorrow}`)
      : "",
    laterMatches.length
  ...
  pushPage(
    ...
    pageShell({
      ...
      description:
        "بث مباشر مباريات اليوم مع جدول اليوم والغد، مواعيد المباريات بتوقيت السعودية، القنوات الناقلة، المعلّقين، النتائج وروابط كل مباراة على كورة زيرو.",
      canonical: "/matches",
      heading: "بث مباشر مباريات اليوم",
      lead:
        "جدول مباريات اليوم والغد في مكان واحد: وقت البداية بتوقيت الرياض، البطولة، القناة، المعلّق والنتيجة عند توفرها، مع رابط مستقل لكل مباراة.",
      body: hubBody,
  ...
  for (const [day, dayMatches] of [...byDay.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    pushPage(
      ...
      pageShell({
        ...
        description: `مواعيد ونتائج مباريات كرة القدم يوم ${day} بتوقيت السعودية مع القنوات والمعلّقين وروابط تفاصيل كل مباراة على كورة زيرو.`,
        canonical: route,
        heading: `مباريات ${day}`,
        lead: "صفحة يومية ثابتة للمواعيد والنتائج والقنوات، مع روابط مباشرة إلى تفاصيل كل مباراة.",
        body: renderMatchSection("جدول المباريات", dayMatches, teamNamesAr, "بتوقيت الرياض"),
  ...
  for (const [team, teamMatches] of eligibleTeams) {
    ...
    pushPage(
      ...
      pageShell({
        ...
        description: `مباريات ${teamAr} اليوم والقادمة مع المواعيد بتوقيت السعودية والنتائج والبطولة والقناة والمعلّق عند توفر البيانات على كورة زيرو.`,
        canonical: route,
  ...
  for (const match of matches) {
    ...
    const details = [
      ...
      `<p><strong>الموعد:</strong> ${escapeHtml(formatKickoffAr(match.kickoffUtc))} بتوقيت الرياض</p>`,
    ...
    pushPage(
      ...
      pageShell({
        title,
        description: `تفاصيل مباراة ${home} ضد ${away}: الموعد بتوقيت السعودية، الحالة، النتيجة، البطولة، القناة والمعلّق عند توفر البيانات.`,
        canonical: route,
        heading,
        lead: `${leagueName(match)} · ${formatKickoffAr(match.kickoffUtc)} بتوقيت الرياض · ${statusLabelAr(match.status)}`,
