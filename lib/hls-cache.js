...
/** Worker Cache API only — client/CDN still get no-store on live manifests. */
export function hlsWorkerCacheTtl(target, segmentTtl = 60) {
  return isLivePlaylistTarget(target) ? 1 : segmentTtl;
}

/** /wk/live and /wk/seg are already uncacheable at the CDN; busting them stalls 0.5s live. */
export function shouldBustHlsProxyUrl(url) {
  const raw = String(url || "");
  try {
    const path = new URL(raw, "https://korazero.com").pathname;
    if (/^\/wk\/(?:live|seg)$/i.test(path)) return false;
  } catch {
    if (/\/wk\/(?:live|seg)(?:\?|$)/i.test(raw)) return false;
  }
  return true;
}
