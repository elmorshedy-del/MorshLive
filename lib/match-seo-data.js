export const SEO_COMPETITIONS = Object.freeze([
  ...
  {
    key: "ucl",
    leagueSlug: "uefa.champions_qual",
...
function compactTeam(competitor, fallbackName = "") {
  ...
  return { id, name, url: firstEntityUrl(team) };
}
  ...
  for (const items of collections) {
    for (const item of Array.isArray(items) ? items : []) {
      const name = cleanText(
        item?.media?.shortName || item?.media?.name || item?.media?.callLetters || item?.name,
      );
      if (!name) continue;
      const row = {
        name,
  ...
  return (Array.isArray(details) ? details : [])
    ...
    .map((detail) => {
      ...
      return {
        ...
        penalty:
          Boolean(detail?.penaltyKick) ||
          /penalty/i.test(cleanText(detail?.type?.text || detail?.type?.type)),
        own:
          Boolean(detail?.ownGoal) || /own goal/i.test(cleanText(detail?.type?.text || detail?.type?.type)),
      };
    })
...
export function normalizeSeoScoreboardEvent(event, leagueSlug) {
  ...
  return {
    ...
    minute:
      status === "live"
        ? cleanText(competition?.status?.displayClock || eventType(competition).shortDetail)
        : "",
...
export function enrichSeoMatchFromSummary(match, summary) {
  ...
  const result = {
    ...
    broadcasters: broadcasterRows(summary.broadcasts, competition.geoBroadcasts, competition.broadcasts)
      .length
      ? broadcasterRows(summary.broadcasts, competition.geoBroadcasts, competition.broadcasts)
      : match.broadcasters,
    headToHead: headToHead(summary),
  };
  result.recentForm = recentForm(summary, result);
...
export function enrichSeoMatch(match, summary) {
  return enrichSeoMatchFromSummary(match, summary);
}
...
export function mergeKnownMatchData(match, known) {
  ...
  for (const field of fields) {
    if (known[field] != null && (Array.isArray(known[field]) ? known[field].length : true))
      out[field] = known[field];
  }
  return out;
}

function stableClean(value) {
  if (value == null) return null;
  if (Array.isArray(value)) return value.map(stableClean);
  if (typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value)
        .filter(([, item]) => item !== undefined)
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([key, item]) => [key, stableClean(item)]),
    );
  }
  return value;
}
...
export function seoPrimaryFingerprint(match) {
  ...
  return JSON.stringify(stableClean(snapshot));
}
...
export function seedSeoMatches(matches, observedAt) {
  ...
  return (Array.isArray(matches) ? matches : []).map((match) => {
    if (match?.seoLastmod || !stamp) return { ...match };
    return { ...match, seoLastmod: stamp };
  });
}
...
export function mergeSeoMatches(previousMatches, currentMatches, observedAt) {
  ...
  const mergedByKey = new Map(
    (Array.isArray(previousMatches) ? previousMatches : []).map((match) => [
      seoMatchKey(match),
      { ...match },
    ]),
  );
  ...
  for (const current of Array.isArray(currentMatches) ? currentMatches : []) {
    ...
    if (!prior) {
      mergedByKey.set(key, stamp ? { ...current, seoLastmod: current.seoLastmod || stamp } : { ...current });
      continue;
    }
    const next = { ...prior, ...current };
    const changed = seoPrimaryFingerprint(prior) !== seoPrimaryFingerprint(next);
    if (changed && stamp) next.seoLastmod = stamp;
    else if (prior.seoLastmod) next.seoLastmod = prior.seoLastmod;
