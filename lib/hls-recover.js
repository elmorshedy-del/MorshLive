...
/**
 * Live edge gap must be >= maxBufferLength or the 30s forward buffer
 * cannot fill. Do not mix with liveSyncDurationCount — hls.js throws.
 */
...
/**
 * hls.js default. Remux on a worker to avoid lag/frame drops
 * (API.md). enableWorker:false is the low-latency knob — it hitch-pauses
 * every TS fragment on the main thread.
 */
export function operatorEnableWorker() {
  return true;
}

/** hls.js default guaranteed forward buffer (API.md maxBufferLength). */
export function operatorMaxBufferLength() {
  return 30;
}

export function operatorMaxMaxBufferLength() {
  return 60;
}

/** Official default. Capping to player size can flip quality mid-live. */
export function operatorCapLevelToPlayerSize() {
  return false;
}
