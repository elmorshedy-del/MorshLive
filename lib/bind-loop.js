...
const ALLOWED_WRAPPER_HOSTS = ["yallacuo.xyz", "koralive1.cc", "koralive.online"];
...
export function isAllowedArabicWrapperUrl(url) {
  ...
  const alba = /\/albaplayer\/[a-z0-9-]+\/?$/i.test(href);
  return alba && ALLOWED_WRAPPER_HOSTS.some((ok) => host === ok || host.endsWith(`.${ok}`));
}

export function productionPlanIsLive(response, expect = {}) {
  const plan = response || {};
  const selected = plan.selected || {};
  const playback = String(selected.playbackUrl || selected.url || "");
