...
export function parseXtreamChannelName(value) {
  ...
  let network = null;
  // Gulf broadcasters first: some catalogue rows are named "BeIN Alkass 1 HD",
  // where "beIN" is a reseller/category prefix rather than the network identity.
  if (/\b(?:al\s*)?kass\b/.test(text) || tokens.includes("alkass")) network = "alkass";
  else if (/\bshasha\b/.test(text)) network = "shasha";
  else if (/\boman\s+sports?\b/.test(text)) network = "oman";
  else if (/\bbe ?in\b/.test(text) || tokens.includes("bein")) network = "bein";
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (/^(bein|sport|sports|max|xtra|alkass|shasha)$/.test(token) || THMANYAH.test(token)) {
      armed = true;
      continue;
    }
...
export function parseSiteChannelId(channelId) {
  ...
  const alkass = /^alkass-([1-8])$/.exec(id);
  if (alkass) return { network: "alkass", tier: "sports", number: Number.parseInt(alkass[1], 10) };

  const shasha = /^shasha-sport-([1-3])$/.exec(id);
  if (shasha) return { network: "shasha", tier: "sports", number: Number.parseInt(shasha[1], 10) };

  if (id === "oman-sports") return { network: "oman", tier: "sports", number: null };
