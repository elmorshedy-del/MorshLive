...
/** Bust already-open live iframes when the operator player boot changes. */
export const OPERATOR_PLAYER_BOOT = "smooth1";
...
export function operatorEmbedProxyPath(href, origin = "") {
  ...
  try {
    ...
    return `${origin}${OPERATOR_EMBED_PATH}?u=${encodeURIComponent(target)}&kz=${OPERATOR_PLAYER_BOOT}`;
  } catch {
    return "";
  }
}

export function shouldRemountOperatorPlayback(loadedUrl, nextUrl) {
  const loaded = String(loadedUrl || "");
  const next = String(nextUrl || "");
  if (!next || !/\/wk\/operator\/?/i.test(next)) return false;
  return loaded !== next;
}
...
/**
 * Returns true when url is an allowed target for scorebug visual verification:
 * - Production KoraZero /wk/operator/ proxy around an allowed operator target
 * - Allowed AlbaPlayer wrapper URL (yallacuo.xyz, koralive1.cc, koralive.online)
 */
export function isScoreBugUrl(url) {
  ...
  try {
    ...
    if (
      ...
      host === "korazero.com" &&
      /^\/wk\/operator\/?$/i.test(parsed.pathname) &&
      isOperatorAlbaPlayerUrl(parsed.searchParams.get("u"))
    ) {
      return true;
    }
  } catch {
  ...
  return isOperatorAlbaPlayerUrl(raw);
}
