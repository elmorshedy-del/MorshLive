 ...
 * fixtures in that UTC window. One timer per ESPN id (T-15, then T-5,
 * then kickoff). Do not arm a finished match.
 */

export const BIND_HORIZON_DAYS = 2;
export const BIND_MATCH_MINUTES = 150;
export const BIND_T15_MINUTES = 15;
export const BIND_T5_MINUTES = 5;
...
/**
 * @returns {null | { check: "t15"|"t5"|"kickoff", executeNow: boolean, fireAt: number }}
 */
export function bindActionForMatch({ kickoffUtc, now = Date.now() } = {}) {
  ...
  if (mins > BIND_T5_MINUTES)
    return { check: "t15", executeNow: true, fireAt: kick - BIND_T15_MINUTES * 60000 };
  if (mins > 0) return { check: "t5", executeNow: true, fireAt: kick - BIND_T5_MINUTES * 60000 };
