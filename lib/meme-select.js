...
// Home scroll: how many posts the rail carries. Widened to a proper feed
// length now that selection keeps each day's posts (minus garbage) instead of
// only a top few.
export const HOME_SCROLL_LIMIT = 30;
export const HOME_SCROLL_POOL_DAYS = 3;

/**
 * Home scroll selection — delegates to the trend ranker (lib/trend-rank.js):
 * a refined timeline. Freshness pool (3 days, 7-day ceiling — never wider;
 * stale filler reads as broken), each day's garbage filtered out, reposts
 * deduped, displayed newest-first like a feed.
 */
