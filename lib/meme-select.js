...
// If the last 3 days are empty (stale ingest), widen the pool in steps so the
// scroll is never blank — always prefers the freshest window that has memes.
const HOME_SCROLL_POOL_WIDEN_DAYS = [3, 7, 14, 30, 0];
...
/**
 * Home scroll selection: take a recent pool (last `poolDays`), keep the `limit`
 * BEST by engagement, then order those by RECENCY (newest first) for display.
 * The pool widens only if a window would be empty, so the scroll stays as fresh
 * as the data allows without ever going blank.
 */
export function selectHomeScrollMemes(memes, opts = {}) {
  const limit = opts.limit || HOME_SCROLL_LIMIT;
  const nowMs = opts.nowMs || Date.now();
  const list = memes || [];
  const windows = opts.poolDays ? [opts.poolDays, 0] : HOME_SCROLL_POOL_WIDEN_DAYS;

  // Narrowest (freshest) window that fills the scroll; widen only as needed so a
  // stale ingest still yields a full scroll instead of a near-empty one.
  let pool = [];
  for (const days of windows) {
    pool = days ? list.filter((m) => memePostedMs(m) >= nowMs - days * 86400000) : list;
