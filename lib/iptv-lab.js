...
/** Channel 2 in that bouquet: `BEIN SPORTS 2 SD`, not ENGLISH / SD² / SD³. */
export function isArBeinSports2SdChannel(channel) {
  const name = String(channel?.name || "").trim();
  if (/english/i.test(name)) return false;
  if (!/bein\s+sports?\s+(?:2\s+sd|sd\s*2)$/i.test(name)) return false;
  return isArBeinSportsSdCategory(channel?.categoryName);
}

export function pickArBeinSports2Sd(channels) {
  const matches = (Array.isArray(channels) ? channels : []).filter(isArBeinSports2SdChannel);
  return matches.find((channel) => String(channel.streamId) === "992") || matches[0] || null;
}

/** HEVC / H.265 bouquet or channel name. Not the H.264 AR beIN SD originals. */
export function isHevcIptvChannel(channel) {
