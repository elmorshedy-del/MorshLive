...
function addArchiveLink(html) {
  ...
  return String(html).replace(
    "</nav>",
    '<a href="/matches/archive">أرشيف المباريات</a></nav>',
  );
}
...
function archiveHtml(matches, siteUrl, teamNamesAr) {
  ...
  const rows = ended
    ...
    .join("\n");
  const description = "أرشيف مباريات كورة زيرو المنتهية مع روابط ثابتة لصفحة كل مباراة والنتيجة والتفاصيل المتاحة.";
  return `<!doctype html>
...
export function buildSeoPages(payload, options = {}) {
  ...
  const archived = Array.isArray(payload?.seoMatches) && payload.seoMatches.length ? payload.seoMatches : payload?.matches || [];
  ...
  const redirectLines = [...new Set([...pages.map((page) => `${page.route}  ${page.file}  200`), "/matches/today  /matches  301"])];
