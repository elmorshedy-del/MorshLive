...
function renderMatchCard(match, teamNamesAr) {
  ...
  const score = match.score && match.score !== "VS" ? `<span class="score">${escapeHtml(match.score)}</span>` : "";
...
export function buildSeoPages(payload, options = {}) {
  ...
  pushPage(
    ...
    pageShell({
      ...
      description: "جدول مباريات اليوم في الدوري الإنجليزي والدوري الإسباني ودوري أبطال أوروبا مع الموعد والنتيجة والقناة والمعلّق عند توفرها.",
  ...
  for (const match of matches) {
    ...
    const details = [
      ...
      match.score && match.score !== "VS" ? `<p><strong>النتيجة:</strong> ${escapeHtml(match.score)}</p>` : "",
    ...
    pushPage(
      route,
      generatedFile("match", `${arabiaDayIso(match.kickoffUtc)}-${slugify(match.home)}-vs-${slugify(match.away)}`),
      pageShell({
