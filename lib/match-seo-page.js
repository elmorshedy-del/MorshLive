...
function statusLabel(match, lang) {
  ...
  if (match?.seoEventStatus === "EventRescheduled")
    return lang === "ar" ? "أعيدت جدولة المباراة" : "Rescheduled";
...
function overviewSection(match, lang) {
  ...
  const rows = [
    ...
    match?.matchday
      ? `<p><strong>${lang === "ar" ? "الجولة" : "Matchday"}:</strong> ${escapeHtml(match.matchday)}</p>`
      : "",
    venue?.name
      ? `<p><strong>${lang === "ar" ? "الملعب" : "Stadium"}:</strong> ${escapeHtml(venue.name)}</p>`
      : "",
    venue?.city
      ? `<p><strong>${lang === "ar" ? "المدينة" : "City"}:</strong> ${escapeHtml(venue.city)}</p>`
      : "",
    venue?.country
      ? `<p><strong>${lang === "ar" ? "الدولة" : "Country"}:</strong> ${escapeHtml(venue.country)}</p>`
      : "",
...
function recentFormSection(match, lang, teamNamesAr) {
  ...
  const cards = [render(home, form.home, match.homeRecord), render(away, form.away, match.awayRecord)].filter(
    Boolean,
  );
  return cards.length
...
function lineupsSection(match, lang, teamNamesAr) {
  ...
  const render = (name, side) =>
    `<div class="seo-card"><h3>${escapeHtml(name)}</h3>${side.formation ? `<p><strong>${lang === "ar" ? "الخطة" : "Formation"}:</strong> ${escapeHtml(side.formation)}</p>` : ""}<ol class="seo-detail">${side.starters.map((player) => `<li>${escapeHtml(player.name)}</li>`).join("")}</ol></div>`;
  const home = lang === "ar" ? arTeam(match.home, teamNamesAr) : match.home;
  const away = lang === "ar" ? arTeam(match.away, teamNamesAr) : match.away;
...
function broadcastersSection(match, lang) {
  ...
  return `<section class="seo-section"><div class="seo-section-head"><h2>${lang === "ar" ? "حقوق البث حسب السوق" : "Broadcast rights by market"}</h2></div><ul class="seo-detail">${rows
    .map(
      (row) =>
        `<li><strong>${escapeHtml(regionName(row.region, lang) || (lang === "ar" ? "السوق المعلن" : "Listed market"))}:</strong> ${escapeHtml(row.name)}${row.type ? ` · ${escapeHtml(row.type)}` : ""}</li>`,
    )
    .join("")}</ul></section>`;
}
...
function coverageSection(match, lang) {
  const rows = [
    match?.channel
      ? `<p><strong>${lang === "ar" ? "القناة" : "Channel"}:</strong> ${escapeHtml(match.channel)}</p>`
      : "",
    match?.commentator
      ? `<p><strong>${lang === "ar" ? "المعلّق" : "Commentator"}:</strong> ${escapeHtml(match.commentator)}</p>`
      : "",
  ].filter(Boolean);
  return rows.length
...
function goalsHtml(match, lang) {
  ...
  return `<div class="seo-card"><h3>${lang === "ar" ? "الأهداف" : "Goals"}</h3><ul class="seo-detail">${goals
    .map((goal) => {
      const flags = [
        goal.penalty ? (lang === "ar" ? "ركلة جزاء" : "penalty") : "",
        goal.own ? (lang === "ar" ? "هدف عكسي" : "own goal") : "",
      ]
        .filter(Boolean)
        .join(" · ");
...
function highlightsHtml(match, lang) {
  ...
  return `<div class="seo-card"><h3>${lang === "ar" ? "الملخص والأهداف" : "Highlights"}</h3><ul class="seo-detail">${rows
    .map(
      (row) =>
        `<li><a href="${escapeHtml(row.videoUrl)}">${escapeHtml(row.title || (lang === "ar" ? "مشاهدة الملخص" : "Watch highlights"))}</a></li>`,
    )
    .join("")}</ul></div>`;
}
  ...
  const title =
    match.status === "ended"
      ...
      : lang === "ar"
        ...
        : "Live score";
  const score =
    match?.score && match.score !== "VS" ? `<p class="seo-score">${escapeHtml(match.score)}</p>` : "";
...
export function buildMatchSeoHtml({
  match,
  route,
