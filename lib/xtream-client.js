...
export const XTREAM_MAX_REDIRECTS = 12;
...
/**
 * Panels that reject byte ranges on a live stream answer 401/403 (auth check
 * keyed on the raw request) or 416 (no range satisfiable on an endless feed).
 * All three mean "ask again without Range", not "this channel is dead".
 */
export function shouldRetryXtreamMediaWithoutRange(status, hadRange) {
  const code = Number(status);
  return Boolean(hadRange) && (code === 401 || code === 403 || code === 416);
}
