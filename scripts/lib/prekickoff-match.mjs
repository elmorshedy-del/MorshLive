...
export async function loadFreshMatches() {
  ...
  try {
    ...
    const { ESPN_LEAGUES, fetchEspnScoreboardWindow, mergeMatches, normalizeEspnEvent } =
      require("../matches-lib.js");
    ...
    const settled = await Promise.allSettled(
      ESPN_LEAGUES.map(async (slug) => {
        const { league, events } = await fetchEspnScoreboardWindow(slug, range, getJson);
        return events.map((e) => normalizeEspnEvent(e, league));
      })
    );
