...
async function choosePlayableThmanyah() {
  ...
  const candidates = (Array.isArray(catalog?.streams) ? catalog.streams : [])
    .filter((channel) => channel?.streamId && channel?.portalId && isThmanyah(channel))
    .sort((a, b) => {
      const aa = thmanyahNumber(a.name) || "9";
      const bb = thmanyahNumber(b.name) || "9";
      return aa.localeCompare(bb) || String(a.name || "").localeCompare(String(b.name || ""));
    });

  if (!candidates.length) {
    throw new Error(`IPTV catalog has ${catalog?.count || 0} streams but no Thmanyah 1/2/3 entry`);
  }

  const attempts = [];
  for (const channel of candidates.slice(0, 12)) {
    ...
    try {
      ...
      attempts.push({ name: channel.name, streamId: channel.streamId, playable: probe?.playable, protocol: probe?.protocol });
      if (probe?.playable && ["ts", "hls"].includes(String(probe.protocol || "").toLowerCase())) {
        return { channel, probe, attempts, catalogCount: catalog?.count || 0, thmanyahCount: candidates.length };
      }
    } catch (error) {
      attempts.push({ name: channel.name, streamId: channel.streamId, error: error?.message || String(error) });
    }
  }

  throw new Error(`No playable Thmanyah catalog stream found: ${JSON.stringify(attempts)}`);
}
...
async function verifyContinuousPlayback(target) {
  ...
  try {
    ...
    const failureState = await page.locator('#player-shell [data-xtream-state="error"], #player-shell [data-xtream-state="timeout"]').count();
    ...
    if (second.readyState < 2) throw new Error(`Thmanyah stream lost decoded media: ${JSON.stringify({ first, second })}`);
    if (second.currentTime < first.currentTime + 8) {
      throw new Error(`Thmanyah playback did not advance continuously for 16 seconds: ${JSON.stringify({ first, second })}`);
    }
